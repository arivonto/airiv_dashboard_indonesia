#!/bin/bash
echo ">>> Safely injecting /demo controller into restored airiv_dashboard_indonesia module <<<"
...
# Safely inject the controllers import into the root __init__.py if it doesn't already exist
if ! grep -q "from . import controllers" "$MODULE_DIR/__init__.py"; then
    echo "from . import controllers" >> "$MODULE_DIR/__init__.py"
fi

echo ">>> Upgrading module to register the endpoint... <<<"
...
echo ">>> Dashboard restored at https://erp.airiv.id/demo <<<"
